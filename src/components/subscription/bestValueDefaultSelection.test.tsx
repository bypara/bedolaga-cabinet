...
// Highlight and selection both remain visible using the configurable accent palette.
const SELECTED = 'bg-accent-500/10';
const BEST_VALUE_RING = 'border-accent-400/70';
...
describe('покупка: выбранный по умолчанию период', () => {
  ...
  it('выбранный выгодный период сохраняет акцентный контур: выбран И выгоден', async () => {
    const harness = await import('./purchase/tariffPurchaseHarness');
    harness.render([
      harness.period({ days: 30, label: '1 месяц' }),
      harness.period({ days: 360, label: '12 месяцев', is_highlighted: true }),
    ...
    expect(screen.getAllByText(ru('subscription.bestValue'))).toHaveLength(1);
  });
...
describe('продление: выбранный по умолчанию период', () => {
  it('выгодный период выбран сразу', async () => {
    ...
    // Both the best-value outline and selected background use admin-controlled colors.
    expect(cardFor(360).style.borderColor).toContain('--color-accent-400');
